...
window.Echo.join("shopowner.channel." + window.userid)
    .here(() => {
    ...
    .listen("Shopownermessage", async (e) => {
        ...
        if (backend.$refs.chatref.userdata.id == e.chatdata.user.id) {
            ...
            if (backend.$refs.chatref.showwrapper) {
                allfrommessagefunction.setreadbyshop(
                    e.chatdata.message.message_user_id
                );
            }
        }
        //for template
        ...
        if (
            localStorage.getItem(
                e.chatdata.message.message_user_id + "_shop_messages"
            ) !== null
        ) {
            //get
            var tmpforls = JSON.parse(
                localStorage.getItem(
                    e.chatdata.message.message_user_id + "_shop_messages"
                )
            );
            ...
            localStorage.setItem(
                e.chatdata.message.message_user_id + "_shop_messages",
                JSON.stringify(tmpforls)
            );
        }
        //for localstrage

        const gslfs =
            await backend.$refs.chatwrapper.getshopschatlistfromserver();
        ...
        const total_chat_count =
            await allfrommessagefunction.gettotalchatcountforshop();
        localStorage.setItem(
            window.userid + "gettotalchatcountforshop",
            total_chat_count
        );
        ...
        var count = JSON.parse(
            localStorage.getItem(window.userid + "getspecificcountforshop")
        );
        count[e.chatdata.message.message_user_id] =
            count[e.chatdata.message.message_user_id] + 1;
        localStorage.setItem(
            window.userid + "getspecificcountforshop",
...
window.Echo.channel("activeusers").listen("ActiveUsers", (e) => {
    ...
    const findex = backend.$refs.chatwrapper.chatlist.findIndex((object) => {
        return object.userdata.id == e.chatdata.users_id;
    });
    if (findex > -1) {
        backend.$refs.chatwrapper.chatlist[findex].userdata.status =
            e.chatdata.status;
    }

    if (localStorage.getItem(e.chatdata.users_id + "_shop_messages") !== null) {
        //retrive ls
        var tmpforlsact = JSON.parse(
            localStorage.getItem(e.chatdata.users_id + "_shop_messages")
        );
        //update status
        tmpforlsact.data.data.userdata.status = e.chatdata.status;
        //set
        localStorage.setItem(
            e.chatdata.users_id + "_shop_messages",
            JSON.stringify(tmpforlsact)
        );
    }
    ...
    if (localStorage.getItem(window.userid + "_schatlist") !== null) {
        //retrive ls
        var tmpforlschatlist = JSON.parse(
        ...
        tmpforlschatlist.data.data[findlsindex].userdata.status = "online";
        //set
        localStorage.setItem(
            window.userid + "_schatlist",
            JSON.stringify(tmpforlschatlist)
        );
    }
});

// for shopowner each other
window.Echo.channel("shopownersmessage." + window.userid).listen(
    "Shopownersmessage",
    async (e) => {
        backend.$refs.chatref.stopscroll = false;
        ...
        if (
            localStorage.getItem(data.message_user_id + "_shop_messages") !==
            null
        ) {
