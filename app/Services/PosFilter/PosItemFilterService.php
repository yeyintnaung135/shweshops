...
class PosItemFilterService
{
    ...
    public function filter_returns($request)
    {
        ...
        if(empty($request->input('fromDate'))){
            $fromDate=Carbon::createFromDate(Carbon::now()->toDateString());

        }else{
            $fromDate = Carbon::createFromDate($request->input('fromDate'));

        }
        if(empty($request->input('toDate'))){
            $toDate =Carbon::createFromDate(Carbon::now()->toDateString())->addDays(1);

        }else{
            $toDate =Carbon::createFromDate($request->input('toDate'))->addDays(1);

        }

        $query = PosReturnList::select('id', 'customer_name', 'phone', 'address', 'gold_fee', 'date', 'category_id')
            ->where('shop_owner_id', $shopId)->whereBetween('created_at', array(
                $fromDate,
               $toDate,
            ));
    ...
    public function filter_incomes($request)
    {
        ...
        $query1 = PosPurchase::join('pos_purchase_sales', 'pos_purchase_sales.purchase_id', '=', 'pos_purchases.id')
            ->select(
                'pos_purchases.id',
                'pos_purchases.name',
                'pos_purchase_sales.qty',
                'pos_purchases.stock_qty',
                'pos_purchases.code_number',
        ...
        $query2 = PosKyoutPurchase::join('pos_purchase_sales', 'pos_purchase_sales.purchase_id', '=', 'pos_kyout_purchases.id')
            ->select(
                'pos_kyout_purchases.id',
                'pos_kyout_purchases.name',
                'pos_purchase_sales.qty',
                'pos_kyout_purchases.stock_qty',
                'pos_kyout_purchases.code_number',
        ...
        $query3 = PosPlatinumPurchase::join('pos_purchase_sales', 'pos_purchase_sales.purchase_id', '=', 'pos_platinum_purchases.id')
            ->select(
                'pos_platinum_purchases.id',
                'pos_platinum_purchases.name',
                'pos_purchase_sales.qty',
                'pos_platinum_purchases.stock_qty',
                'pos_platinum_purchases.code_number',
        ...
        $query4 = PosWhiteGoldPurchase::join('pos_purchase_sales', 'pos_purchase_sales.purchase_id', '=', 'pos_white_gold_purchases.id')
            ->select(
                'pos_white_gold_purchases.id',
                'pos_white_gold_purchases.name',
                'pos_purchase_sales.qty',
                'pos_white_gold_purchases.stock_qty',
                'pos_white_gold_purchases.code_number',
