<template>
  <div class="col-md-12 pt-3">
    ...
    >
      <label style="font-size: 15px">အနည်းဆုံး ပုံ တစ်ပုံတင်ပါ</label>
    </div>
    ...
    <vue-dropzone
      ...
      id="customdropzone"
    >
      <div class="dropzone-custom-content yk-mt-lg mb-2" style="text-align: center">
        <i
...
export default {
  ...
  checkhasdefaultimage: function () {
    for (var s = 0; s < document.getElementsByClassName("dz-profile-pic").length; s++) {
      if (
        document.getElementsByClassName("dz-profile-pic")[s].style.display == "inline"
      ) {
        return true;
        break;
      }
  ...
  removefilefromdz: function (file, error, xhr) {
    ...
    const tkey = this.thumb_photos.findIndex((tp) => tp.name === file.upload.filename);
    this.thumb_photos.splice(tkey, 1);
    //mid
    const mkey = this.mid_photos.findIndex((mp) => mp.name === file.upload.filename);
    ...
    get_file_key = this.tempphotonames.findIndex((re) => re === file.upload.filename);
    ...
    if (this.checkhasdefaultimage()) {
      ...
      if (document.getElementsByClassName("dz-profile-pic").length != 1) {
        if (document.getElementsByClassName("dz-profile-pic").length == 0) {
          ...
          if (get_file_key == 0) {
            ...
            if (get_file_key !== 0) {
              this.setdefaultphoto(this.tempphotonames[get_file_key - 1]);
            } else {
              this.setdefaultphoto(this.tempphotonames[get_file_key + 1]);
            }
          }
        ...
        this.setdefaultphoto(document.getElementsByClassName("dz-profile-pic")[0].id);
  ...
  thumbnail: function (file, dataUrl) {
    ...
    if (file.previewElement) {
      ...
      refp = file.previewElement.querySelectorAll("[yk-dz-default-pic]");
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (this.$refs.myVueDropzone.getAcceptedFiles().length === 1) {
          ...
          if (this.checkhasdefaultimage()) {
            ...
            this.setdefaultphoto(
              this.$refs.myVueDropzone.getAcceptedFiles()[0].upload.filename
            );
          }
        }
        ...
        for (var cf = 0; cf < this.$refs.myVueDropzone.getAcceptedFiles().length; cf++) {
  ...
  sendingEvent(files, xhr, formData) {
    ...
    formData.append("forthumbphotos", JSON.stringify(this.thumb_photos));
    ...
    formData.append("အထည်မပျက်_ပြန်သွင်း", this.undamaged_product);
    formData.append("အထည်ပျက်စီးချို့ယွင်း", this.damaged_product);
    formData.append("တန်ဖိုးမြင့်အထည်_နှင့်_အထည်မပျက်ပြန်လဲ", this.valuable_product);
    formData.append("gold_quality", document.getElementsByName("gold_quality")[0].value);
    formData.append("gold_colour", document.getElementsByName("gold_colour")[0].value);
    ...
    formData.append("weight", JSON.stringify(this.$refs.aa.weight_child));
