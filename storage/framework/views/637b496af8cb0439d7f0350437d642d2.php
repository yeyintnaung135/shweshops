...
<?php $__env->startPush('scripts'); ?>
    <script>
        ...
        var custompopular = $('[name="custom_popular"]');
        ...
        custompopular.on('change', function() {
            var parentDiv = document.getElementById(this.id).parentNode;
            var divId = parentDiv.querySelector("input[type=hidden]").id;
            var action;
            if (this.checked) {
        ...
        if (custom_popular.length > 0) {

            $(custompopular).prop('checked', true);
        } else {

            $(custompopular).prop('checked', false);
        }
