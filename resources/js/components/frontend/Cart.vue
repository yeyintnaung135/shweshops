...
import Vue from "vue";
import { allservicesfromcart } from "./services/cart";
Vue.use(VueLazyload);
...
export default {
    ...
    async mounted() {
        ...
        if(this.checkloginnow){
           const tmpuploadafter= await this.upload_fav_localstorage_to_server_after_logined();
           this.busy=false;
        }
        if('cart' == 'favourite'){
            this.uri='myfav';
    ...
    methods: {
        ...
        deletecart: function (itemid) {
            console.log("delete");
            allservicesfromcart.start_process(itemid, this);
            ...
            if (
                ...
                if (
                   typeof localStorage.getItem("cart_rm") !== 'undefined' &&
                    ...
                    var tmp_rm = localStorage.getItem("cart_rm");
                }
                return new Promise((resolve, reject) => {
                    axios
                        .post(this.host + "/mycart/upload_after_logined", {
        ...
        get_fav_items_data: function (cart_ids) {
        
            return new Promise((resolve) => {
                axios
                    .post(this.host + "/mycart/get_cart_items_data", {
                        cart_ids: cart_ids,
                    })
                    .then(async (response) => {
                        resolve(response);
        ...
        get_fav_items_data_authuser: function (fav_ids) {
            ...
            return new Promise((resolve) => {
                axios
                    .post(this.host + "/mycart/get_cart_items_data_authuser", {})
                    .then(async (response) => {
                        resolve(response);
                    });
            });
